...
msg_fin:    .asciiz "\nFIN DEL PROGRAMA."
  ...
  #   for (int i = 0; i < n; i++) { std::cout << v[i] << separador; }
  #}
print_vec:  # Funcion para imprimir un vector, recibe como parametros:
  ...
  sw $s4, 16($sp) # guardamos el valor de $s4
  sw $s3, 12($sp)
  sw $s2, 8($sp)
  sw $s1, 4($sp)
  sw $s0, 0($sp)
  ...
  lw $s1, 4($sp)
  lw $s2, 8($sp)
  lw $s3, 12($sp) # recuperamos el valor de $s3
  lw $s4, 16($sp) # recuperamos el valor de $s4
  lw $ra, 20($sp) # recuperamos el valor de $ra
...
print_vec_fin: # Correcto
  ...
  # void change_elto(float* v, int n, float valor) { v[n] = valor; }
...
swap: # Funcion para intercambiar elementos, recibe como parámetros:
  ...
  mul $t3, $t1, size # i * size
  add $t3, $t3, $t0 # t3 = v + i
  l.s $f4, 0($t3) # f4 = v[i] (aux)

  mul $t4, $t2,  size# j * size
  add $t4, $t4, $t0 # t4 = v + j
  l.s $f6, 0($t4) # f6 = v[j]
...
mirror: # Funcion para invertir un vector, recibe como parametros:
        # $a0: direccion del vector
        # $a1: dimension del vector
  add $sp $sp -4 # reservamos espacio para los registros $t0-$t2 y $ra
  sw $ra, 0($sp) # guardamos el valor de $ra
  ...
  intercambio:
    ...
    lw $ra, 0($sp) # recuperamos el valor de $ra
    addi $sp, $sp, 4 # liberamos espacio para guardar el valor de $ra, v, n
...
mirror_fin: # Correcto
  ...
  # float mult_add(float vec1, float vec2, int result) { return vec1 * vec2 + result; }
...
mult_add_fin: # Correcto
  ...
  #   // sin recursividad
  #   float result = 0;
  #   for (int i = 0; i < n; i++) { result = mult_add(v1[i], v2[i], result);}
  #   return result;
  # }
...
main:
  ...
  Menu: # Bucle principal del programa
    Show_vectors:
      ...
      li $v0, 4 #std::cout << "Vector con dimension " << n1 << std::endl;
      la $a0, cabvec
      syscall
      li $v0, 1
      move $a0, $s2
      ...
      li $v0, 4 #std::cout << std::endl;
      la $a0, newline
      syscall

      li $v0, 4 #std::cout << "Vector con dimension " << n2 << std::endl;
      la $a0, cabvec
      syscall
      li $v0, 1
      move $a0, $s3
    ...
    Switch: # Bucle para elegir la opcion elegida
      ...
      CambiarDimension:
        ...
        CambiarDimension1:
          ...
          j Menu
        CambiarDimension1_fin:

        CambiarDimension2:
          move $s3, $t1 # n2 = dimension
          sw $s3, n2 # n2 = dimension
          j Menu
        CambiarDimension2_fin:

        Vector_error:
          li $v0, 4 #std::cout << "Error: Opcion no valida" << std::endl;
          la $a0, error_op
          syscall
          j Menu
        Vector_error_fin:
        ...
        Dimen_error:
          li $v0, 4 #std::cout << "Error: Dimension no valida" << std::endl;
          la $a0, error_dim
          syscall
          j Menu
        Dimen_error_fin:
      CambiarDimension_fin: # Cambiar dimension de los vectores perfecto
      ...
      CambiarElemento:
        ...
        Vector_error2:
          ...
          j Menu
        Vector_error2_fin:
      CambiarElemento_fin: # Cambiar elemento de los vectores perfecto
      ...
      InvertirVector:
        ...
        Vector_error3:
          ...
          j Menu
        Vector_error3_fin:
      InvertirVector_fin: # Invertir vector perfecto
      ...
      ProductoEscalar:
        ...
        error_prod:
          ...
          j Menu
        error_prod_fin:
      ProductoEscalar_fin: # Producto escalar perfecto
    Switch_fin:
  Menu_fin:
FinPrograma:
li $v0, 4
