...
menu:       .ascii  "\n\n(1) Cambiar dimensiones\n(2) Intercambiar dos elemento\n"
            ...
            .ascii  "(5) Calcula la suma de la primera ultima fial y compararlos\n(6) Calcula la suam de la submatriz superior\n"
            .asciiz "(0) Salir\n\nElija opcion: "
...
msg_mod11: .asciiz "\nLa suma de la primera fila es mayor que la suma de la ultima fila.\n"
msg_mayor: .asciiz " > "
msg_mod12: .asciiz "\nLa suma de la segunda fila es mayor que la suma de la primra fila.\n"
msg_menor: .asciiz " < "
msg_mod13: .asciiz "\nLa suma de la primera fila es igual que la suma de la ultima fila.\n"
msg_igual: .asciiz " = "

msg_mod21: .asciiz "\nLa suma de los valores por encima de la diagonal principal sin incluirla es: "
    .text
main:
  ...
  Menu:
    ...
    switch:
      ...
      #   case 5: modificacion1
        beq $s1, 5, Modificacion1
      #   case 6: modificacion2
        beq $s1, 6, Modificacion2
      #   case 0:
        beqz $s1, Fin
      #   default:
        b ErrorOpcion
      ...
      Modificacion1:
        ...
        mul $s4, $t1, size # ncol * size = size_salto
        mul $s5, $t0, $s4 # nfil * size_salto = size_matriz
        move $s1, $zero # suma1 = 0
        move $s2, $zero # suma2 = 0
        move $s3, $zero # i = 0
        #   for (int i = 0; i < nfil; i++){
        For5:   move $s4, $zero # j = 0
          #     for (int j = 0; j < ncol; j++){
            For6:
          #       if (i == 0 || i == nfil - 1){
            If4:   
              beq $s2, $zero Suma1 # if (i == 0)
              sub $t2, $s5, size # nfil - 1
              beq $s2, $t2 Suma2 # if (i == nfil - 1)
              j If4_fin

              Suma1:
                addu $t4, $s2, $s4 # i * size_salto
                addu $t4, $t4, $s0 # i * size_salto + j
                lw $t5, 0($t4)
                addu $s1, $s1, $t5 # suma1 += mat[i*ncol + j]
                j Suma2_Fin
              Suma1_Fin:
              Suma2:
                addu $t4, $s2, $s4 # i * size_salto
                addu $t4, $t4, $s0 # i * size_salto + j
                lw $t5, 0($t4)
                addu $s2, $s2, $t5 # suma2 += mat[i*ncol + j]
              Suma2_Fin:
            If4_fin:
            addu $s3, $s3, size # j++
            blt $s3, $s4, For4
        ...
        EsMenor:
          # std::cout << "\nLa suma de la primera fila es menor que la suma de la ultima fila.\n" << suma1 << " < " << suma2 << "\n";
          li $v0, 4
          la $a0, msg_mod12
          syscall
          li $v0, 1
      ...
      Modificacion2:
        ...
        move $s2, $zero # i = 0
        #   for (int i = 0; i < nfil; i++){
        For7:   move $s3, $zero # j = 0
        #     for (int j = 0; j < ncol; j++){
          For8:
        #       if (f < c){
            If5:  
              blt $s2, $s3 Sumar # if (i < j)
              j Fin_If5
        #         suma += mat[i*ncol + j];
              Sumar2:    
                addu $t4, $s2, $s3 # i * size_salto
